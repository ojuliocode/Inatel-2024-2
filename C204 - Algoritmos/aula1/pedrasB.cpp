...
int escolhe_pedras_dc(pedra pedras[], int n, int i, int cap_peso, int cap_volume, char side){
    cout << side;
    cout << i << endl;
	...
	solucao1 = 0 + 
	escolhe_pedras_dc(pedras, n, i+1, cap_peso, cap_volume, 'l');
	...
	if(nova_cap_peso >= 0 && nova_cap_volume >= 0){
		...
		escolhe_pedras_dc(pedras, n, i+1, nova_cap_peso, nova_cap_volume, 'r');
	}
...
int main(){
	...
	cout << escolhe_pedras_dc(pedras, n, 0, 400, 1000, 'f') << endl;
